...
const get3DPadAndOutInfo =
    (pad: number|string|number[], inDepth: number, inHeight: number, inWidth: number, strideDepth: number, strideHeight: number,
     ...
     filterWidth: number): {padInfo: PadInfo3D; outDepth: number; outHeight: number; outWidth: number} => {
      ...
      if (typeof pad === 'number') {
        ...
        // TODO: handle 'SAME_UPPER'and 'SAME_LOWER'.
    ...
    (inputs: readonly TensorView[], attributes: ConvAttributes, outputShape: readonly number[],
     filterDims: readonly number[], pads: readonly number[], dataFormat: string): ProgramInfo => {
      ...
      return {
        name: 'Conv3DNaive',
        shaderCache:
            {hint: `${attributes.cacheKey};${isChannelsLast};${innerElementSize};${isVec4};${hasBias}`, inputDependencies},
        getRunData: () => ({
